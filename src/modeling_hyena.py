...
class HyenaDNAForTokenClassification(HyenaDNAPreTrainedModel):
    config_class = HyenaConfig  # Config class for PreTrainedModel
    base_model_prefix = "hyena"  # Prefix for saving/loading weights

    def __init__(self, config, **kwargs):
        super().__init__(
            config, **kwargs
        )  # Initialize parent (handles weight init, etc.)
        self.num_labels = kwargs.get(
            "num_labels", config.num_labels
        )  # Priority: kwargs > config
        self.hyena = HyenaDNAModel(config)  # Backbone: HyenaDNA model
        self.classifier = nn.Linear(
            config.d_model, self.num_labels, bias=False
        )  # Token-level classifier
        self.criterion = nn.CrossEntropyLoss(
            ignore_index=-100, weight=torch.tensor([0.7, 2.0])
        )
        self.post_init()  # Final weight init
